...
class RunReport:
    ...
    media_images: List[Path] = field(default_factory=list)
    media_video: Path = None
    ...
    def summary(self) -> str:
        ...
        if self.media_images:
            lines.append(f"Media: {len(self.media_images)} image card(s)"
                         + (f" + video ({self.media_video.name})" if self.media_video else ""))
...
class ContentPipeline:
    ...
    def run(
        ...
        # Generate all text first (the video script drives the voiceover),
        # then build the day's media bundle once and attach it.
        generated: List[GeneratedContent] = []
        for ct in types:
            if ct not in CONTENT_TYPES:
                log.warning(f"Skipping unknown content type '{ct}'")
                continue
            generated.append(generate(ct, self.cfg, report.snapshot, force_provider))

        media = None
        if self.cfg.media.enable_images or self.cfg.media.enable_video:
            from .media import build_media

            script = next(
                (c.body for c in generated if c.content_type == "video_script"), None
            )
            media_dir = self.cfg.output_dir / f"media-{report.snapshot.as_of}"
            media = build_media(report.snapshot, self.cfg, media_dir, script)
            report.media_images = list(media.images)
            report.media_video = media.video

        for content in generated:
            if media:
                if content.content_type in ("daily_brief", "social_thread", "newsletter"):
                    content.images = list(media.images)
                if content.content_type in ("social_thread", "video_script"):
