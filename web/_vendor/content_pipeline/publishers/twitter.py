...
class TwitterPublisher(BasePublisher):
    ...
    def _upload_image(self, path) -> str:
        """Best-effort image upload via the v2 media endpoint (requires
        media.write scope). Returns a media id or raises."""
        ...
        return str(data.get("id") or data.get("media_id_string"))

    def _post_tweet(self, text: str, reply_to: str = None, media_id: str = None) -> dict:
    ...
    def publish(self, content: GeneratedContent) -> PublishResult:
        ...
        media_id = None
        if content.images and content.images[0].exists():
            try:
                media_id = self._upload_image(content.images[0])
            except Exception:
                media_id = None  # text-only is fine
        try:
            first = self._post_tweet(tweets[0], media_id=media_id)
            prev_id = first["id"]
            for t in tweets[1:]:
